...
dem_rep_precinct <- left_join(clean_dem_rep, precinct_voting_locations, by = c("county" = "county_code", "election_district_precinct" = "precinct_code"))
...
dem_rep_precinct%>%
  mutate(total_votes = election_night_votes + early_votes+ provisional_votes)
...
dem_rep_precinct%>%
  ...
  summarize(precinct_votes = sum(total_votes))%>%
  arrange(desc(precinct_votes))
...
dem_rep_precinct%>%
  mutate(total_votes = election_night_votes + early_votes+ provisional_votes)%>%
  group_by(election_district_precinct)%>%
  summarize(mean(county), precinct_votes = sum(total_votes))%>%
  arrange((precinct_votes))
...
**There are 8 precincts, with most from county 17, Prince George's County**

What else would you need to know about these precincts to know whether this might be worth a story? *Put answer here*

**I would want to know more about the demographics of these precincts--education levels, race, etc. to see if there is a connection to lack of voting.**
...
**Which candidate received the most total votes?**
...
dem_rep_precinct%>%
  ...
  summarize(candidate_votes = sum(total_votes))%>%
